...
// Builds an in-memory ropeBWT3 rld (FMD) from a run-length BWT given as Movi-style
...
// so the smallest byte (e.g. the 0 sentinel) becomes code 0 and is treated as the
// endmarker by ConstructPsi/runInfoFromFMD -- matching the byte collation used to
// build the BWT (e.g. libsais/divsufsort on Movi's %-separated text). This lets
// TeraLCP ingest BWTs over a non-DNA alphabet (e.g. with a '%' separator) that the
// ropeBWT3 builder cannot represent.
    ...
    if (o.otsv != "") {
        ...
        if (mode == TeraLCP::RunLCPMode::thresholds) {
            ...
            if (o.inputFormat == options::rlbwt) {
                // The index constructor freed the fmi we built; rebuild a fresh rld
                // from the same heads/len files to read run metadata.
                rb3_fmi_t fmiThr;
                rb3_fmi_init(&fmiThr, buildRldFromRlbwt(o.inputFile), 0);
            ...
            try {
                // Fast path: fuse threshold capture into the parallel per-run
                // traversal. It is destructive (frees Phi/Psi/PLCPsamples), so if
                // -orlcp is also requested (which needs those structures) fall back
                // to the non-destructive phi-walk path.
                if (o.orlcp == "") {
                    ourIndex.writeThresholdsParallel(o.otsv, o.threshbound, runInfo, o.v);
                } else {
            ...
            // Emit Movi-compatible run-length BWT companion files (.bwt.heads,
            // .bwt.len) in the same run partition as the thresholds, so a Movi
            // index can be built from the ropeBWT3 BWT + Tera thresholds. Skipped
            // for -f rlbwt input, where the heads/len already exist (the input)
            // and the code->char map would not match the remapped alphabet.
            if (o.inputFormat != options::rlbwt) {
                try {
                    ourIndex.writeBwtHeadsLen(o.otsv, runInfo);
                } catch (const std::exception& e) {
                    std::cerr << "ERROR: " << e.what() << std::endl;
                    exit(1);
                }
